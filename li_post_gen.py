...
def generate_post(topic, tone, persona, mode="full", real_life_note="", style_note="", post_type=""):
    ...
    Use advanced storytelling frameworks such as:
    - PAS (Problem → Agitate → Solution)
    - Relatable analogies or vivid language
    - Reflective or emotional tone to build connection

    The post must include:
    - A powerful hook (question, bold claim, or relatable opening)
    - A real insight or learning based on experience
    - An actionable or reflective takeaway
    - A human, conversational CTA (no salesy language)

    Format: Hook → Story/Insight → Takeaway → CTA
    Tone: {tone}
    Length: {'2-4 lines' if mode == 'quick' else '8-12 lines'}
    Style: Conversational, bold, value-driven, human — not robotic. {style_note}

    {f"Incorporate this real-life scenario or insight: {real_life_note}" if real_life_note else ""}
    """
